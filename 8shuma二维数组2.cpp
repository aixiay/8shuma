...
int jieguo[3][3]={{1,2,3},{8,0,4},{7,6,5}};//结果状态 
...
struct weizhi  //定义不同状态的结构体 
{
	...
	void dai()//计算代价 
	{
		...
		for(int i=0;i<3;i++)
		{
			for(int j=0;j<3;j++)
			{
				if(erwei[i][j]!=jieguo[i][j])  //如果与结果不同，代价加一 
					c++;
			}
	...
	bool operator<(const weizhi<T>& w)const //重载“<”，优先选择代价小的状态 
	{
		return daijia>w.daijia;
	}
};
/* 
...
int shuma<T>::bfs()
{
	...
	mm[num]=1;
	//定义结构体队列 
	priority_queue<weizhi<int> > q;	
	q.push(wei);  //将第一个状态放入队列 

	//当队列不为空时 
	while(!q.empty())
	{
		 wei=q.top(); // weizhi<int>
		 wei.dai();
	//	取出队首并删除 
		q.pop();
...
void shixian()
{
	...
	while(cin>>n)
	{
		...
		for(int i=0;i<=9;i++)
			jishu[i]=0;
		for(int i=0;i<3;i++)//输入初始状态并统计0-8的个数
		{
			for(int j=0;j<3;j++)
			{
				cin>>a[i][j];
		...
		else
		{
			...
			cout<<"请按1继续..."<<endl;
		 //2 8 3 1 0 4 7 6 5      8 7 6 5 4 3 2 1 0
		}
		//逆序奇偶同性可互达,不包括0 
	}
}
